...
from networks import NoisyDQN, NoisyDQNConvNet
...
class CartPoleNoisyDqnAgent(DQNAgent):
    ...
    def __init__(self, input_type: str, **kwargs):
        ...
        """
        try:
            super(CartPoleNoisyDqnAgent, self).__init__(
    ...
    def load_model(cls, model_dir: Union[str, os.PathLike]):
        ...
        if input_type == "vector":
            agent.dqn_model = NoisyDQN.custom_load(model_data)
        else:
            agent.dqn_model = NoisyDQNConvNet.custom_load(model_data)
    ...
    def init_vector_agent(
        ...
        self.dqn_model = NoisyDQN(state_size, self.env.action_space.n, **kwargs).to(self.device)
        self.dqn_target = NoisyDQN.custom_load(self.dqn_model.custom_dump()).to(self.device)
    ...
    def init_image_agent(
        ...
        self.dqn_model = NoisyDQNConvNet(height=screen_height, width=screen_width, channels=screen_channels,
                                         action_dim=self.env.action_space.n, **kwargs).to(self.device)
        self.dqn_target = NoisyDQNConvNet.custom_load(self.dqn_model.custom_dump()).to(self.device)
    ...
    def train_noisy_dqn_batch(self, batch) -> float:
        ...
        self.dqn_model.reset_noise()
        self.dqn_target.reset_noise()

        return loss.item()

    def select_action(self, state: torch.Tensor) -> Union[torch.Tensor, Tuple[torch.Tensor, bool]]:
        """
    ...
    def run(self, output_dir: Union[str, os.PathLike] = None) -> None:
        ...
        for episode_num in pbar:
            ...
            while True:
                ...
                if episode_num % int(self.num_episodes * 0.05) == 0:
                    action, is_greedy = self.select_action(state)
                ...
                if t_total % self.policy_update == 0:
                    ...
                    loss = self.train_noisy_dqn_batch(batch)
                ...
                if done:
                    ...
                    pbar.set_description(
                        ...
                        f' {episode_return:5.2f} | Avg len: {avg_len}'
                    )
                    break
        ...
        # Save the model
        dump_loc = self.save_trained_results(output_dir, "noisy_dqn_model_" + self.input_type, best_model=best_model)
