...
class CartPoleNoisyDqnAgent(DQNAgent):
    ...
    def init_image_agent(
        ...
        # Initialize the optimizer
        self.optimizer = torch.optim.Adam(self.dqn_model.parameters())
        self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=0)
        self.loss_fn = "mse"
    ...
    def run(self, output_dir: Union[str, os.PathLike] = None) -> None:
        ...
        # Save the model
        dump_loc = self.save_trained_results(output_dir, "noisy_dqn_model_" + self.input_type)
