...
class CartPoleDoubleDqnAgent:
    ...
    def save_trained_results(self, output_dir: Union[str, os.PathLike] = None) -> str:
        ...
        model_dir = os.path.join(output_dir, curr_datetime + "_double_dqn_model")
        os.mkdir(model_dir)
    ...
    def train_dqn_batch(optimizer, batch, dqn_model, dqn_target, gamma) -> float:
        ...
        target_action = torch.argmax(dqn_model(non_final_next_states), dim=1).detach()

        target_values = torch.zeros(batch.rewards.shape)
        target_values[~batch.dones] = target_preds.gather(1, target_action.view(-1, 1)).squeeze(1).detach()
    ...
    def run(self, output_dir: Union[str, os.PathLike] = None) -> None:
        ...
        for t_total in pbar:
            ...
            if t_total % 4 == 0:
                ...
                loss = CartPoleDoubleDqnAgent.train_dqn_batch(self.optimizer, batch, self.dqn_model,
