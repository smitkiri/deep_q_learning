...
class DQNConvNet(nn.Module):
    def __init__(self, height, width, channels, action_dim, *, num_conv_layers=3, num_fc_layers=3, hidden_dim=256,
        ...
        for idx in range(num_conv_layers):
            ...
            output_shape = DQNConvNet.conv_output_shape(output_shape, kernel_size=kernel_size, stride=stride)
        ...
        for idx in range(num_fc_layers):
            ...
            # Add a linear layer
            fc_layers.append((f"linear{idx + 1}", nn.Linear(liner_input_size, output_size)))
